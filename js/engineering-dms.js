document.addEventListener('DOMContentLoaded', async () => {
    const contentDiv = document.getElementById('dms-content');
    if (!contentDiv) return;

    try {
        const data = await fetchAllData();
        const dmsData = data.filter(r => r.cycle === 'Engineering' && r.specialty === 'DMS');

        if (dmsData.length === 0) {
            contentDiv.innerHTML = '<p>No DMS data found.</p>';
            return;
        }

        const years = groupBy(dmsData, 'year');
        ...
        console.error('Error loading DMS data:', error);
